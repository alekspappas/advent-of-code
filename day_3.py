...
grid_one = createGrid(wire_one)
grid_two = createGrid(wire_two)
intersection = grid_one.intersection(grid_two)
print(intersection)

sums = []
for val in intersection:
    sums.append(abs(val[0])+abs(val[1]))
...
def createDictGrid(directions):
    ...
    for i in range(len(directions)):
        ...
        for j in range(int(directions[i][1:])):
            ...
            if firstLetter == "R":
                state = (state[0] + 1, state[1])
                add_to_dict(points_dict,(state[0] + 1, state[1]),count)
            elif firstLetter == "L":
                state = (state[0] - 1, state[1])
                add_to_dict(points_dict,(state[0] - 1, state[1]),count)
            elif firstLetter == "U":
                state = (state[0],state[1] + 1)
                add_to_dict(points_dict,(state[0],state[1] + 1),count)
            elif firstLetter == "D":
                state = (state[0],state[1] - 1)
                add_to_dict(points_dict,(state[0],state[1] - 1),count)
    return points_dict

grid_one = createDictGrid(wire_one)
grid_two = createDictGrid(wire_two)
intersection = {}

for key in grid_one:
    if key in grid_two:
        intersection[key] = grid_one[key] + grid_two[key]

key_min = min(intersection.keys(), key=(lambda k: intersection[k]))

print intersection[key_min]
